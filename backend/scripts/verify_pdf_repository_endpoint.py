...
def main():
    ...
    print(f"Verifying GET /pdf/repository for: {repo_url}")
    ...
    try:
        ...
        # 1. Run repository analysis to populate cache
        ...
        # 2. Verify cache has the analysis snapshot
        ...
        # 3. Call PDF repository endpoint
        client = TestClient(app)
        
        # We will mock GitHubService and LLMService during the GET /pdf/repository call
        # to ensure that NO GitHub or LLM requests are made.
        print("Calling GET /pdf/repository endpoint (with mocked external services)...")
        ...
        with patch("app.services.github_service.GitHubService.get_repo_metadata", side_effect=assert_no_external_call), \
            ...
            response = client.get(f"/pdf/repository?owner={owner}&repo={repo}")
            
        if response.status_code != 200:
            print("\n-------------------------------------------------")
            print("Status:\nFAILURE")
            print(f"Error: PDF Repository endpoint returned status code {response.status_code}")
            print(f"Detail: {response.text}")
            print("-------------------------------------------------")
            sys.exit(1)
            
        # 4. Verify headers and content
        ...
        expected_filename = f"{repo.lower()}_repository_guide.pdf"
