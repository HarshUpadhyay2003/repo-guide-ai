...
def main():
    ...
    print(f"Verifying GET /pdf/contribution for: {repo_url}")
    ...
    try:
        ...
        # 3. Call PDF contribution endpoint
        client = TestClient(app)
        
        # We will mock GitHubService and LLMService during the GET /pdf/contribution call
        # to ensure that NO GitHub or LLM requests are made.
        print("Calling GET /pdf/contribution endpoint (with mocked external services)...")
        ...
        with patch("app.services.github_service.GitHubService.get_repo_metadata", side_effect=assert_no_external_call), \
            ...
            response = client.get(f"/pdf/contribution?owner={owner}&repo={repo}")
            
        if response.status_code != 200:
            print("\n-------------------------------------------------")
            print("Status:\nFAILURE")
            print(f"Error: PDF Contribution endpoint returned status code {response.status_code}")
        ...
        expected_filename = f"{repo.lower()}_contribution_guide.pdf"
