  ...
  return (
    <div className="flex flex-col gap-6 rounded-xl border border-white/5 bg-[#111217] p-6 shadow-lg hover:border-[#8B5CF6]/30 hover:shadow-[0_0_24px_rgba(139,92,246,0.10)] transition-all duration-300 sm:p-8 min-w-0">
      <h2 className="flex items-center gap-2 text-xl font-bold text-slate-100">
        <Map className="h-6 w-6 text-[#8B5CF6] shrink-0" />
        Exploration Hints
      </h2>
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        ...
        <div className="flex flex-col gap-4 min-w-0">
          <h3 className="flex items-center gap-2 border-b border-white/5 pb-2 text-xs font-bold uppercase tracking-wider text-slate-500">
            <Folder className="h-4 w-4 shrink-0" /> Likely Directories
          </h3>
          <ul className="flex flex-col gap-2.5">
            {hints.likely_directories.map((dir) => (
              <li key={dir} className="flex items-start gap-2.5 rounded-lg bg-black/30 p-2.5 text-sm font-medium text-slate-300 border border-white/5 font-mono min-w-0">
                <Folder className="h-4 w-4 text-[#8B5CF6] shrink-0 mt-0.5" />
        ...
        <div className="flex flex-col gap-4 min-w-0">
          <h3 className="flex items-center gap-2 border-b border-white/5 pb-2 text-xs font-bold uppercase tracking-wider text-slate-500">
            <FileCode className="h-4 w-4 shrink-0" /> Possible Files
          </h3>
          <ul className="flex flex-col gap-2.5">
            {hints.possible_files.map((file) => (
              <li key={file} className="flex items-start gap-2.5 rounded-lg bg-black/30 p-2.5 text-sm font-medium text-slate-300 border border-white/5 font-mono min-w-0">
                <FileCode className="h-4 w-4 text-[#8B5CF6] shrink-0 mt-0.5" />
      ...
      {/* AI Reasoning */}
      <div className="mt-4 flex items-start gap-4 rounded-xl border border-[#2EC55E]/20 bg-[#2EC55E]/10 p-5 min-w-0">
        ...
        <div className="flex flex-col gap-1.5 min-w-0">
          ...
          <p className="text-sm leading-relaxed text-slate-300 break-words">{hints.reasoning}</p>
        </div>
      </div>
    </div>
  );
