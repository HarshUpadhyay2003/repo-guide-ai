      ...
      <div className="flex flex-col gap-3 min-w-0">
        {files.map((file, idx) => (
          <div key={idx} className="flex flex-col gap-2 rounded-xl border border-white/5 bg-black/30 p-4 min-w-0">
            <div className="flex items-start gap-2 text-sm font-mono text-[#8B5CF6] min-w-0">
              <FileCode className="h-4 w-4 shrink-0 mt-0.5" /> 
              <span className="min-w-0 break-all sm:break-words [overflow-wrap:anywhere]" title={file.path}>{file.path}</span>
            </div>
            <p className="text-sm text-slate-400 leading-snug font-sans break-words">{file.reason}</p>
          </div>
        ))}
      </div>
    </div>
